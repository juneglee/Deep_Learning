...
#             # 음성데이터의 decode와 mel과의 차원의 갯수를 맞춰주기 위해서 자리수를 맞춘다
#             max_target_len += self.n_frames_per_step - max_target_len % self.n_frames_per_step
#
#         mel_padded = torch.FloatTensor(len(batch), num_mels, max_target_len)
#         mel_padded.zero_()
#         # 멈춰야할지 구분하기 위해서 label을 이용하여 사용한다. (0과 1)
#         # stop token layer에서 멈추는 것을 1로 멈추지 않는것을 0으로 만들어준다
#         # mel이 없는 공간은 1로 채워준다
#         gate_padded = torch.FloatTensor(len(batch),max_target_len)
#         gate_padded.zero_()
#         output_lengths = torch.LongTensor(len(batch))
#         output_lengths.zero_() # LongTensor로 만들면 랜덤이 사용되기 때문에 zero로 사용한다.
#
#
#         for i in range(len(ids_sorted_decreasing)):
#             mel = batch[ids_sorted_decreasing[i]][0]
#             mel_padded[i, :, :mel.size(1)] = mel
#             gate_padded[i, mel.size(1) - 1 :] = 1 # -1 : 가장 마지막의 데이터도 예측하기 위해서 사용
#             output_lengths[i] = mel.size(1)
#
#         print(gate_padded)
#         return text_padded, output_lengths, mel_padded, input_lengths
#
...
#     # drop_last : 배치 사이즈로 실행한 나눈 짜투리 데이터를 버리겠다.
#
